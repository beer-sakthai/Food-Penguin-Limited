...
export interface AppOptions {
  aiClientFactory?: () => {
    models: {
      ...
      generateImages: (args: any) => Promise<{
...
function getAiClient(aiClientFactory?: AppOptions["aiClientFactory"]) {
  if (aiClientFactory) return aiClientFactory();
...
async function generateText(
  ...
  aiClientFactory?: AppOptions["aiClientFactory"],
) {
  const ai = getAiClient(aiClientFactory);
  ...
  app.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const text = await generateText(
        command,
        "Answer concisely for a restaurant operations dashboard.",
        options.aiClientFactory,
      );
      res.json({ text });
    }),
  );
  ...
  app.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (workflow === "generate-marketing-image") {
        ...
        const model = getStringBodyValue(
          body,
          "model",
        ...
        const ai = getAiClient(options.aiClientFactory);
