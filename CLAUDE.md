...
- `src/firebase.ts` — **not Firebase at all**: the `firebase` npm package is not a dependency. It's a `localStorage`-backed emulator exposing a Firestore/Auth-shaped surface (`auth`, `db`, `signOut`, `onAuthStateChanged`, `setDoc`, `doc`, `getDocs`, `onSnapshot`, `handleFirestoreError`, `OperationType`) so the app persists data locally/offline while keeping familiar call sites. Writes are validated per collection (`validateProposedDoc`) and never leave the browser.
