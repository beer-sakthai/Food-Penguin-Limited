      ...
      <div className="xl:col-span-2 space-y-4 min-h-0 overflow-y-auto scrollbar-hide">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm">
          ...
          <div className="h-40 mt-6 mb-4">
            <ResponsiveContainer width="100%" height="100%">
               <BarChart data={statusData} layout="vertical" margin={{ top: 0, right: 0, left: 10, bottom: 0 }}>
                 ...
                 <YAxis type="category" dataKey="status" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 10, fontWeight: 'bold' }} />
                 <Tooltip 
                   cursor={{ fill: '#f8fafc' }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-4 border-t border-slate-100">
            {tasks.map((task) => (
              <div key={task.id} className="border border-slate-200/80 rounded-xl p-4 bg-slate-50 flex justify-between items-start">
                <div className="space-y-1">
                  ...
                  <h4 className="text-sm font-bold text-slate-800">{task.itemName}</h4>
                  <p className="text-xs text-slate-400 font-mono">Station: {task.assignedTo} | {task.quantity} items</p>
                </div>

                <div className="space-y-2 text-right">
                  <span className={`inline-block px-2.5 py-1 rounded-full text-[10px] font-mono font-bold ${
                    task.status === 'Prepared' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 
                    task.status === 'Cooking' ? 'bg-amber-50 text-amber-700 animate-pulse' : 'bg-slate-100 text-slate-600'
                  }`}>
                    {task.status}
                  </span>
                  ...
                  <div className="flex gap-1 justify-end font-sans">
                    {task.status === 'In Queue' && (
                      <button
                        ...
                        className="p-1 px-2 border text-[10px] rounded bg-white hover:bg-slate-100 text-slate-700"
                      >
                        Cook
                      </button>
                    )}
                    {task.status === 'Cooking' && (
                      <button
                        ...
                        className="p-1 px-2 border text-[10px] rounded bg-white hover:bg-slate-100 text-slate-700 font-semibold"
                      >
                        Finish
                      </button>
                    )}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {recipes.map((rcp) => (
              <div key={rcp.id} className="flex justify-between items-center border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                <div className="space-y-1">
                  <h4 className="text-sm font-bold text-slate-800">{rcp.name}</h4>
                  <div className="flex flex-wrap gap-1 items-center">
                    <span className="text-[10px] font-mono text-slate-400">Prep: {rcp.prepTime} mins | Allergens:</span>
                    {rcp.allergens.map((alg, i) => (
                      <span key={i} className="px-1 text-[9px] bg-rose-50 text-rose-600 rounded">{alg}</span>
                    ))}
                  </div>
                </div>
      ...
      <div className="space-y-4 min-h-0 overflow-y-auto scrollbar-hide">
        ...
        <div className="bg-white rounded-xl border border-slate-150 p-5 shadow-sm">
          <div className="flex items-center gap-2 pb-4">
            ...
            <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">Deploy Kitchen Work-slip</span>
          </div>

          <form onSubmit={handleSubmitTask} className="space-y-3">
            <div>
              <label className="text-[10px] font-mono text-slate-400 uppercase">Select Specialty item</label>
              <select
                required
                value={taskItem}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[10px] font-mono text-slate-400 uppercase">Station cook</label>
                <select
                  value={assignedTo}
              ...
              <div>
                <label className="text-[10px] font-mono text-slate-400 uppercase">Quantity (pcs)</label>
                <input
                  type="number"
                  min="1"
                  required
            ...
            <div>
              <label className="text-[10px] font-mono text-slate-400 uppercase">Priority Index</label>
            ...
            <button
              type="submit"
              className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white font-medium text-xs rounded transition-colors inline-flex justify-center items-center gap-1.5 shadow"
            >
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-bold text-slate-900 uppercase tracking-widest">Kitchen Quality Auditor</span>
            </div>
          ...
          <div className="space-y-1.5">
            <span className="text-[10px] uppercase font-mono tracking-wider text-slate-400 font-bold block">Preset Simulation Plates:</span>
            <div className="grid grid-cols-3 gap-2">
              {SAMPLE_MOCK_DISHES.map((d, i) => (
                <button
                  ...
                  className="bg-white border border-slate-200 hover:border-sky-500 rounded p-1.5 text-left transition-all text-[10px] flex flex-col justify-between"
                >
                  <span className="font-semibold text-slate-700 truncate block">{d.name}</span>
                  <span className="text-[8px] text-slate-400 block truncate mt-1">Select dish</span>
                </button>
              ))}
            </div>
          </div>

          <div className="border border-dashed border-slate-300 rounded-lg p-4 bg-white hover:bg-slate-50 text-center transition-all">
            <input
              type="file"
              ref={fileInputRef}
            ...
            {selectedPhoto ? (
              <div className="space-y-2">
                <div className="flex justify-center max-h-36 overflow-hidden rounded bg-slate-900 p-2 border">
                ...
                <div className="flex justify-between items-center text-[10px]">
                  <span className="text-emerald-600 font-mono truncate px-1 max-w-[150px]">{fileLabel}</span>
                  <button 
              ...
              <button 
                ...
                className="w-full flex flex-col items-center justify-center p-2 text-slate-500 hover:text-sky-500 gap-1"
              >
          ...
          <button
            ...
            className="w-full py-2 bg-slate-900 border border-slate-800 disabled:bg-slate-350 hover:bg-slate-800 text-white text-xs font-medium rounded transition-colors inline-flex justify-center items-center gap-2 shadow-sm"
          >
            {auditorLoading ? (
              <>
          ...
          {auditResult && (
            <div className="bg-white border border-slate-200/95 rounded-lg p-4 shadow-inner space-y-2">
              <span className="text-[10px] uppercase font-mono tracking-wider text-rose-600 font-bold block">Optical Scan Analysis:</span>
              <p className="text-xs text-slate-700 leading-relaxed font-sans whitespace-pre-wrap">{auditResult}</p>
            </div>
          )}
        </div>
