    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-sans">
          {targets.map((tgt) => {
            ...
            return (
              <div key={tgt.id} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
                <div className="flex justify-between items-start pb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-[10px] font-mono uppercase text-slate-400 tracking-widest font-bold">{tgt.category} goal</h4>
                      <h3 className="text-sm font-sans font-bold text-slate-900 group-hover:text-orange-500 transition-colors mt-0.5">{tgt.name}</h3>
                    </div>
                  </div>
                  <span className={`px-2.5 py-0.5 rounded-full font-mono text-[10px] ${
                    isCompleted ? 'bg-emerald-50 text-emerald-700 font-bold border border-emerald-100' : 'bg-orange-50 text-orange-700 border border-orange-100'
                  }`}>
                ...
                <div className="space-y-2 mt-4">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-400 font-mono">Metric: {tgt.metric}</span>
                  ...
                  <div className="flex justify-between items-center text-[10px] text-slate-400 font-mono pt-1">
                    ...
                    <span className="text-slate-600 font-bold">{Math.round(isHours ? (isCompleted ? 100 : 50) : pct)}% Done</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-sans font-bold text-slate-900 flex items-center gap-1.5">
                ...
                <span className="px-1.5 py-0.2 rounded bg-slate-200 text-slate-600 text-[9px] font-mono">
                  gemini-3.1-flash-lite
                </span>
              </h3>
              <p className="text-xs text-slate-500">Formulate and test mathematical target limits before deployment</p>
            </div>
          </div>
          ...
          <textarea
            ...
            className="w-full h-16 p-3 text-xs bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-orange-500 font-sans shadow-inner"
          />
          ...
          <div className="text-right">
            <button
              ...
              className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-350 text-white font-bold text-xs rounded-xl transition-all inline-flex items-center gap-2 shadow-md"
            >
              {loadingSuggestion ? (
                <>
          ...
          {aiSuggestions && (
            <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-inner space-y-2">
              <span className="text-[10px] uppercase font-mono tracking-wider text-orange-600 font-bold block">Optimized Target Guidelines:</span>
              <p className="text-xs text-slate-705 leading-relaxed font-sans">{aiSuggestions}</p>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm max-h-[550px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold">Target Header Name</label>
            <input
              type="text"
              required
              value={name}
          ...
          <div>
            <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold">Assessment Metric</label>
            <input
              type="text"
              required
              value={metric}
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <div>
              <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold">Target value</label>
              <input
                type="number"
                required
                value={targetValue || ''}
                onChange={(e) => setTargetValue(parseInt(e.target.value) || 0)}
                className="w-full mt-1.5 p-2.5 border border-slate-200 rounded-xl text-xs focus:ring-1 focus:ring-orange-500 focus:outline-none font-mono"
              />
            </div>
            <div>
              <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold">Unit label</label>
              <input
                type="text"
                required
                value={unit}
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <div>
              ...
              <select
                ...
                className="w-full mt-1.5 p-2.5 border border-slate-200 bg-white rounded-xl text-xs focus:ring-1 focus:ring-orange-500 focus:outline-none"
              >
            ...
            <div>
              <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold">Target Deadline</label>
              <input
                type="text"
                required
                value={deadline}
          ...
          <button
            type="submit"
            className="w-full py-2.5 mt-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-colors inline-flex items-center justify-center gap-1.5 shadow-md"
          >
