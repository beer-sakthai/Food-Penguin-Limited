...
import { autoFlagInventory, getDayContributingItems } from "./appUtils";
...
export default function App() {
  ...
  const handleAddOrder = async (newOrder: Partial<SalesOrder>) => {
    ...
    if (
      !newOrder ||
      typeof newOrder.item !== 'string' ||
      typeof newOrder.quantity !== 'number' ||
      typeof newOrder.amount !== 'number' ||
      !['Completed', 'Pending', 'Refunded'].includes(newOrder.status as any)
    ) {
  ...
  const handleAddTarget = async (newTarget: Partial<CompanyTarget>) => {
    ...
    if (
      !newTarget ||
      typeof newTarget.name !== 'string' ||
      typeof newTarget.metric !== 'string' ||
      typeof newTarget.targetValue !== 'number' ||
      typeof newTarget.currentValue !== 'number' ||
      !['Sell', 'Production', 'Waste', 'Hours'].includes(newTarget.category as any)
    ) {
  ...
  const handleAddTask = async (newTask: Partial<ProductionTask>) => {
    ...
    if (
      !newTask ||
      typeof newTask.itemName !== 'string' ||
      typeof newTask.assignedTo !== 'string' ||
      typeof newTask.quantity !== 'number' ||
      !['In Queue', 'Cooking', 'Prepared', 'Out for Delivery'].includes(newTask.status as any)
    ) {
  ...
  const handleAddWaste = async (newWaste: Partial<WasteRecord>) => {
    ...
    if (
      !newWaste ||
      typeof newWaste.item !== 'string' ||
      typeof newWaste.weight !== 'number' ||
      typeof newWaste.cost !== 'number' ||
      !['Expired', 'Overproduced', 'Quality Issue', 'Spill/Accident'].includes(newWaste.reason as any)
    ) {
  ...
  return (
    ...
    >
      <div
        className={`h-full w-full overflow-hidden rounded-[28px] border shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_24px_80px_rgba(0,0,0,0.35)] ${isLight
      ...
      >
        <div className="h-full w-full grid grid-cols-1 md:grid-cols-[minmax(180px,17vw)_minmax(0,1fr)] xl:grid-cols-[minmax(180px,17vw)_minmax(0,1fr)_minmax(260px,22vw)]">
          ...
          >
            {/* Global Toolbar */}
            <header
              className={`h-16 px-4 md:px-6 flex items-center justify-between sticky top-0 z-30 transition-all duration-200 border-b ${isLight
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span
                  className={`hidden lg:inline-block text-xs font-mono px-2 py-0.5 rounded uppercase tracking-wider font-bold border ${isLight
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${isLight
          ...
          <aside
            className={`w-full h-full flex flex-col shrink-0 border-b md:border-b-0 md:border-r transition-all duration-300 ${isMobileMenuOpen ? "fixed inset-0 z-50 h-[100dvh] overflow-hidden md:relative md:inset-auto" : "relative z-40"} ${isLight
          ...
          >
            ...
            >
              <div className="md:hidden">
                ...
                <div className={`mx-4 mt-4 p-3 gold-liner-box transition-all ${isLight ? "bg-amber-50/20" : "bg-zinc-950/40"
                  }`}>
                    ...
                    {(
                      ...
                      return (
                        <div
                          ...
                          className={`flex justify-between items-center text-[10px] p-2 rounded-lg border cursor-pointer transition-colors ${isSelected
                              ? isLight
                        ...
                        >
                          <span
                            className={`truncate mr-2 ${isSelected
                                ? isLight
                ...
                {tabMeta.map((tab) => {
                  ...
                  return (
                    <button className={`btn-interactive w-full text-left py-2.5 px-3.5 rounded-xl text-xs font-semibold flex items-center gap-3 transition-colors duration-200 ${isActive ? isLight ? "bg-zinc-100 text-zinc-950 font-extrabold shadow-sm" : "bg-zinc-900 text-white font-bold shadow-inner" : isLight ? "text-zinc-600 text-zinc-600 hover:-translate-y-0.5 hover:shadow-lg active:scale-[0.98] hover:bg-zinc-50 hover:text-zinc-900" : "text-zinc-500 hover:bg-zinc-905 hover:text-white"}`} key={tab.id} onClick={() => {
                      ...
                      <span
                        className={`w-2 h-2 rounded-full transition-all duration-300 shrink-0 ${isActive
                            ? tab.id === "Real-time"
              ...
              <div className="px-2 py-1.5">
                <div
                  className={`p-3 rounded-xl border relative overflow-hidden group transition-all duration-200 ${isLight
                      ? "bg-zinc-50 border-zinc-200 text-zinc-900 shadow-sm"
                      : "bg-zinc-900 border-zinc-800 text-white"
                    }`}
                >
                  <div
                    className={`absolute right-0 top-0 w-24 h-24 bg-gradient-to-br rounded-full filter blur-2xl pointer-events-none ${isLight
                        ? "from-orange-500/5"
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <button className={`btn-interactive flex items-center gap-1.5 transition-colors cursor-pointer text-left focus:outline-none ${isLight ? " hover:-translate-y-0.5 hover:shadow-lg active:scale-[0.98] hover:text-zinc-900" : "hover:text-white"}`} onClick={() => setIsCapacityExpanded(!isCapacityExpanded)} title="Click to view daily breakdown">
                      <p
                        className={`text-xs uppercase font-mono font-bold tracking-wider select-none ${isLight ? "text-zinc-500" : "text-zinc-400"
                          }`}
                      >
                        Weekly Capacity
                      </p>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        className={`flex items-center gap-1 text-xs font-mono font-bold px-1.5 py-0.5 rounded border select-none transition-all ${aiAccuracyConfidence >= 90
                            ? isLight
                      ...
                      >
                        <span
                          className={`w-1 h-1 rounded-full ${aiAccuracyConfidence >= 90
                  ...
                  <div
                    className={`flex flex-col gap-2 mt-1 mb-2.5 p-2 rounded-lg font-mono text-xs select-none border transition-all ${isLight
                  ...
                  <div
                    className={`relative h-3 rounded-full overflow-hidden mt-3 shadow-inner ${isLight ? "bg-zinc-200" : "bg-zinc-800/80"
                      }`}
                  >
                    {/* Visual 'Safe Zone' range marker (40% to bottleneckThreshold) */}
                    {bottleneckThreshold > 40 && (
                      <motion.div
                        className={`absolute top-0 h-full border-l border-r border-dashed z-0 ${isLight
                  ...
                  <div
                    className={`space-y-1.5 mt-3 pt-2.5 border-t font-mono text-xs leading-relaxed ${isLight ? "border-zinc-200" : "border-zinc-800/60"
                      }`}
                  >
                    ...
                    {overlayBranches.length > 0 && (
                      <div
                        className={`mt-2 pt-2 border-t border-dashed space-y-1.5 ${isLight ? "border-zinc-200" : "border-zinc-800/40"
                          }`}
                      >
                        <p
                          className={`text-xs uppercase tracking-wider font-bold mb-1 select-none ${isLight ? "text-zinc-400" : "text-zinc-500"
                            }`}
                        >
                        ...
                        {Object.entries(branchProjectionData).map(([name, val]) => {
                          ...
                          return (
                            ...
                            >
                              <span
                                className={`flex items-center gap-1.5 ${isLight ? "text-zinc-600" : "text-zinc-300"
                    ...
                    <p
                      className={`text-xs leading-normal mt-1 pt-1 italic font-sans border-t ${isLight
                  ...
                  {isCapacityExpanded && (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            className={`p-1 px-1.5 rounded  hover:-translate-y-0.5 hover:shadow-lg active:scale-[0.98] hover:text-white transition-all cursor-pointer flex items-center gap-1 border ${isLight
                      ...
                      <div
                        className={`flex flex-col gap-2 p-2.5 rounded-xl border ${isLight
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center justify-between gap-1">
                            <span
                              className={`text-xs font-bold uppercase tracking-widest flex items-center gap-1 ${quickAdjustEnabled
                      ...
                      <div
                        className={`flex flex-col gap-2 p-2.5 rounded-xl border relative ${isLight
                      ...
                      >
                        <div className="flex justify-between items-center text-xs text-zinc-500 font-bold uppercase tracking-widest leading-none">
                          ...
                          <motion.span
                            ...
                            className={`font-mono text-xs font-bold px-1.5 py-0.5 rounded border transition-colors duration-200 ${isLight
                        ...
                        {showThresholdTooltip && (
                          <div
                            className={`absolute bottom-full left-0 right-0 mb-2 p-3.5 rounded-xl border shadow-2xl z-50 transition-all font-sans text-xs font-normal normal-case tracking-normal leading-relaxed ${isLight
                      ...
                      <div
                        className={`p-2.5 rounded-xl border ${isLight
                            ? "bg-zinc-100 border-zinc-200"
                            : "bg-zinc-950/80 border-zinc-900/60"
                          }`}
                      >
                      ...
                      {(() => {
                        ...
                        if (hasBottleneck) {
                          ...
                          return (
                            <div
                              className={`w-full rounded-xl p-3.5 mb-3 border flex flex-col gap-3 transition-all duration-300 relative ${isLight
                            ...
                            >
                              ...
                              <div className="pl-1.5 flex flex-col gap-2">
                                ...
                                {/* Loading State */}
                                {quickFixLoading && (
                                  <div className={`p-3 rounded-lg border flex flex-col items-center justify-center gap-2 ${isLight ? "bg-amber-100/30 border-amber-200" : "bg-amber-950/20 border-amber-900/30"
                                    }`}>
                                ...
                                {(quickFixRecommendation && isQuickFixForThisDay) && (
                                  <div className={`p-2.5 rounded-lg border flex flex-col gap-2.5 transition-all duration-300 ${isLight
                                    ...
                                    <div className="flex items-center justify-between gap-1 border-t pt-2 border-zinc-200/50 dark:border-zinc-850/50">
                                      ...
                                      <div className="flex items-center gap-1.5">
                                        ...
                                        <button
                                          ...
                                          className={`px-2 py-1 rounded text-xs font-extrabold uppercase tracking-wider transition-all hover:-translate-y-0.5 active:scale-[0.98] shadow-sm flex items-center gap-1 border focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 cursor-pointer ${isLight
                      ...
                      >
                        {(() => {
                          ...
                          if (filteredDailyCapacityBreakdown.length === 0) {
                            return (
                              <div className={`flex flex-col items-center justify-center py-8 px-4 text-center rounded-xl border ${isLight
                          ...
                          return filteredDailyCapacityBreakdown.map((item, index) => {
                            ...
                            return (
                              <Reorder.Item
                                ...
                                className={`flex flex-col gap-1.5 pb-2 last:border-0 last:pb-0 transition-all duration-700 ${isLight
                                    ? "border-zinc-200"
                              ...
                              >
                                ...
                                {compareModeEnabled && (
                                  <motion.div
                                    ...
                                    className={`mt-1.5 p-1.5 rounded-lg border flex flex-col gap-1 overflow-hidden transition-all duration-300 ${isLight
                                  ...
                                  >
                                    <div className="flex justify-between items-center text-xs font-mono leading-none">
                                      ...
                                      <div className="flex items-center gap-0.5 font-bold">
                                        ...
                                        <span
                                          className={`px-1 rounded-sm ${item.projected > item.initialAiForecast
                                    ...
                                    <div className="h-1.5 bg-zinc-950 dark:bg-zinc-950/90 rounded-full relative overflow-hidden flex items-center">
                                      ...
                                      <motion.div
                                        className={`absolute h-full rounded ${item.projected >= item.initialAiForecast
                                      ...
                                      <motion.div
                                        className={`absolute w-0.5 h-2 rounded-full z-10 ${item.projected >= item.initialAiForecast
                                            ? "bg-amber-400"
                                            : "bg-rose-400"
                                          }`}
                                ...
                                {quickAdjustEnabled && (
                                  <div
                                    className={`mt-1.5 flex flex-wrap items-center justify-between gap-1.5 p-1.5 rounded-xl border transition-all duration-200 ${isLight
                                ...
                                {expandedDays.includes(item.day) && (
                                  <motion.div
                                    ...
                                    className={`mt-2 overflow-hidden text-xs border-t pt-2 space-y-1.5 ${isLight
                                        ? "border-zinc-200 shadow-inner"
                                        : "border-zinc-800/60 shadow-inner"
                                      }`}
                                  >
                                    <div
                                      className={`font-semibold flex items-center justify-between px-1 mb-1 ${isLight
                                    ...
                                    <div className="space-y-1">
                                      {getDayContributingItems(
                                        ...
                                        return (
                                          <div
                                            key={idx}
                                            className={`flex items-center justify-between p-1.5 rounded-lg border transition-all duration-200 hover:scale-[1.01] ${isLight
                      ...
                      <div
                        className={`mt-3 pt-2.5 border-t flex flex-wrap items-center justify-center gap-x-4 gap-y-1.5 text-xs ${isLight
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1">
                          <span
                            className={`w-2 h-2 rounded-sm border ${isLight
              ...
              {/* Aesthetic & Theme Panel */}
              <div className={`mx-2 mt-1.5 p-2.5 gold-liner-box transition-all ${isLight ? "bg-amber-50/20" : "bg-zinc-950/40"
                }`}>
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div
                    className={`w-8 h-8 rounded-full flex flex-col items-center justify-center text-zinc-300 relative shrink-0 border overflow-hidden ${isLight
          ...
          >
            {/* Global Toolbar */}
            <header
              className={`h-16 px-6 flex items-center justify-between sticky top-0 z-30 transition-all duration-200 border-b ${isLight
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span
                  className={`hidden lg:inline-block text-[9px] font-mono px-2 py-0.5 rounded uppercase tracking-wider font-bold border ${isLight
                      ? "bg-zinc-100 text-zinc-600 border-zinc-200"
                      : "bg-zinc-900 text-zinc-400 border-zinc-800"
                    }`}
                >
                  ...
                  {(
                    ...
                    return (
                      <div
                        key={branch}
                        onClick={() => setSelectedBranch(branch)}
                        className={`flex justify-between items-center text-xs p-1.5 rounded-lg border cursor-pointer transition-colors ${isSelected
                            ? isLight
                              ? "bg-orange-50 border-orange-200"
                              : "bg-orange-500/10 border-orange-500/30"
                            : isLight
          ...
          <aside
            className={`hidden xl:flex h-full min-h-0 w-full flex-col shrink-0 border-l transition-colors duration-300 overflow-hidden ${isLight
          ...
          >
            <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-4 py-4 space-y-4">
              <div
                className={`mx-0 p-3 gold-liner-box transition-all ${isLight ? "bg-amber-50/20" : "bg-zinc-950/40"
                  }`}
              >
                <div className="flex items-center gap-1.5 mb-2.5 pb-1.5 border-b border-yellow-500/20">
                  <Sparkles className="w-3.5 h-3.5 text-yellow-500 animate-pulse shrink-0" />
              ...
              <div
                className={`mx-0 p-3 gold-liner-box transition-all ${isLight ? "bg-zinc-50 shadow-sm" : "bg-zinc-900 shadow"
                  }`}
              >
                <div
                  className={`flex items-center gap-2 mb-3 pb-2 border-b ${isLight ? "border-zinc-200" : "border-zinc-800/80"}`}
      ...
      {isScheduleReportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm animate-fadeIn">
          <div
            className={`w-full max-w-sm rounded-[1.25rem] shadow-2xl p-6 relative border animate-zoomIn ${isLight
                ? "bg-white border-zinc-200"
                : "bg-zinc-950 border-zinc-800"
              }`}
          >
