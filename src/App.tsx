...
export default function App() {
    ...
    return (
      <button
        ...
        className={`w-full text-left py-2 px-3 rounded-lg text-xs font-medium flex items-center gap-2.5 transition-colors ${
          isActive
            ? (isLight ? "bg-zinc-100 text-zinc-900" : "bg-zinc-800 text-white")
  ...
  return (    <div className={`min-h-screen flex ${isLight ? "bg-zinc-50 text-zinc-900" : "bg-zinc-950 text-zinc-100"}`}>      <aside className={`w-60 shrink-0 flex flex-col border-r ${isLight ? "bg-white border-zinc-200" : "bg-zinc-900 border-zinc-800"}`}>        <div className="p-4 border-b border-inherit">          <div className="flex items-center gap-3">            <div className="w-8 h-8 rounded-lg bg-zinc-900 text-white flex items-center justify-center text-sm font-bold">FP</div>            <div className="flex-1 min-w-0">              <p className="text-sm font-bold leading-tight truncate">Food Penguin</p>              <p className="text-[10px] text-zinc-500">Limited · {healthLabel}</p>            </div>          </div>        </div>        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">          {primaryTabs.map((tab) => renderNavigationButton(tab))}          {operationsTabs.length > 0 && (            <div className="pt-4 mt-2 space-y-1 border-t border-inherit">              {operationsTabs.map((tab) => renderNavigationButton(tab))}            </div>          )}        </nav>        <div className={`p-3 border-t border-inherit text-[11px]`}>          <div className="flex items-center justify-between">            <span className="font-medium truncate">{currentUser?.username || "Skipper Koala"}</span>            <select value={userRole} onChange={(e) => setUserRole(e.target.value as any)} className="bg-transparent text-zinc-500 text-[10px] font-mono uppercase cursor-pointer focus:outline-none appearance-none">              <option value="Admin">Admin</option>              <option value="Manager">Manager</option>              <option value="Staff">Staff</option>              <option value="User">User</option>            </select>          </div>        </div>      </aside>      <div className="flex-1 flex flex-col min-w-0">        <header className={`h-14 px-5 flex items-center justify-between border-b ${isLight ? "bg-white border-zinc-200" : "bg-zinc-900 border-zinc-800"}`}>          <div className="flex items-center gap-4">            <h1 className="text-sm font-bold">{tabMeta.find((t) => t.id === activeTab)?.label || activeTab}</h1>            <select value={selectedBranch} onChange={(e) => setSelectedBranch(e.target.value as any)} className="bg-transparent text-zinc-500 text-xs font-medium focus:outline-none cursor-pointer">              <option value="Marks & Spencer - Cork City">M&S Cork</option>              <option value="Tesco - Cork City">Tesco Cork</option>              <option value="Tesco - Mahon Point">Tesco Mahon</option>            </select>          </div>          <div className="flex items-center gap-3">            <span className="text-xs font-mono text-zinc-500">{irelandTime || "—"}</span>            <button type="button" onClick={toggleTheme} className={`p-1.5 rounded-md ${isLight ? "hover:bg-zinc-100 text-zinc-500" : "hover:bg-zinc-800 text-zinc-400"}`}>{isLight ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}</button>          </div>        </header>        <main className="flex-1 p-6 overflow-y-auto">          <div className="max-w-7xl mx-auto">            <AnimatePresence mode="wait" initial={false}>              <motion.div key={activeTab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.15 }}>                {renderActiveView()}              </motion.div>            </AnimatePresence>          </div>        </main>      </div>    </div>  );}